...
CREATE TABLE IF NOT EXISTS `academicos`.`user` (
  ...
  INDEX `fk_user_class1_idx` (`class_idclass` ASC),
...
CREATE TABLE IF NOT EXISTS `academicos`.`frequencia` (
  ...
  `user_class_idclass` INT(11) NOT NULL,
  `discipline_iddiscipline` INT(11) NOT NULL,
  PRIMARY KEY (`idfrequencia`, `user_iduser`, `user_class_idclass`, `discipline_iddiscipline`),
  INDEX `fk_frequencia_user1_idx` (`user_iduser` ASC, `user_class_idclass` ASC),
  INDEX `fk_frequencia_discipline1_idx` (`discipline_iddiscipline` ASC),
  CONSTRAINT `fk_frequencia_user1`
    FOREIGN KEY (`user_iduser` , `user_class_idclass`)
    REFERENCES `academicos`.`user` (`iduser` , `class_idclass`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
